import { TOPIA_GITHUB_LINK } from '../../constants/community'
export const HomeEnterSection = () => {
  return (
    <section className="g-page-height flex w-full justify-center">
      <div className="g-body-container flex h-full items-center">
        <div className="ml-[10px] h-[500px] w-[260px] md:ml-0 md:mt-[184px] md:w-[550px]">
          <div className="flex justify-between text-[50px] font-bold leading-[74px] tracking-wide md:text-[108px] md:leading-[95px]">
            <h1>SCALE</h1>
            <h1>TO</h1>
          </div>
          <h1 className="text-[50px] font-bold md:text-[108px]">INFINITY</h1>
          <h2 className="mt-[22px] w-[299px] text-[28px] font-thin leading-[37px] tracking-[1px] md:mt-0 md:w-[602px] md:leading-[40px] md:tracking-[1px]">
            <p className="block md:hidden">The First dStorage</p>
            <p className="block md:hidden">Layer2 solution</p>
            <p className="hidden md:block">
              The first dStorage Layer2 solution
            </p>
          </h2>
          <a
            href={TOPIA_GITHUB_LINK}
            target="_blank"
            rel="noreferrer"
            className="mt-[37px] flex w-[158px] items-center justify-center rounded-full bg-[#6eff89] py-[10px] text-[13px] text-black md:mt-[44px] md:w-[148px] md:py-[10px] md:text-[16px]"
          >
            Start Building
          </a>
        </div>
      </div>
    </section>
  )
}
